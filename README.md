    ...
    <div id="dashboardSection">
        ...
        <div id="tabWallet" class="tab-content hidden">
            <div class="card" style="background: #e0f2f1;">
                ...
                <p style="font-size: 12px; text-align: center; color: #666;">User ID: <span id="myUserIdDisplay" style="font-family: monospace; background: #fff; padding: 2px 4px; border-radius: 3px;">Loading...</span></p>
        ...
        <div id="adminPanelSection" class="card hidden" style="border: 2px solid var(--accent); background: #fff;">
            ...
            <div class="admin-section-box">
                <h4>1. Kisi bhi User ko Points Bhejein (Only Admin)</h4>
                <input type="text" id="adminTargetUserId" placeholder="User ki ID dalein (jaise: user_xxxx)">
            ...
            <div class="admin-section-box">
                ...
                <input type="text" id="groupTeamsInput" placeholder="Teams comma se alag karein (jaise: DSG, JSK)">
            ...
            <div class="admin-section-box">
                ...
                <label style="font-size: 12px; font-weight: bold; color: #333; display: block; margin-top: 5px;">Action Chunein:</label>
                ...
                <div id="existingTableSection">
                    ...
                    <input type="text" id="ptCustomNrrInput" placeholder="Optional: NRR adjust (e.g. +1.245)" style="margin-top:8px;">
                ...
                <div id="newTableSection" class="hidden">
                    <input type="text" id="newPtTableName" placeholder="Nayi Table/Group Name" style="margin-top:2px;">
                    <input type="text" id="newPtTableTeams" placeholder="Teams comma se dalein (MI, CSK, RCB)" style="margin-top:6px;">
...
<script>
    ...
    const ADMIN_MY_OWN_ID = "User_malik076"; // Aapki fixed user id

    function initUserSession() {
        let myUserId = localStorage.getItem('cricket_user_id');
        if(!myUserId) {
            myUserId = ADMIN_MY_OWN_ID;
        ...
        db.ref('users/' + myUserId).once('value', (snapshot) => {
            ...
            if(!snapshot.exists()) {
                db.ref('users/' + myUserId).set({
                    points: 5000, 
                    subscription: "Lifetime Admin Plan",
                    subExpiry: currentTime + (365 * 24 * 60 * 60 * 1000)
                });
            } else {
                let userData = snapshot.val();
                let updates = {};
                if(!userData.points || userData.points < 5000) {
                    updates.points = 5000;
                }
                if(!userData.subExpiry || userData.subExpiry <= currentTime) {
                    updates.subscription = "Lifetime Admin Plan";
                    updates.subExpiry = currentTime + (365 * 24 * 60 * 60 * 1000);
                }
                if(Object.keys(updates).length > 0) {
                    db.ref('users/' + myUserId).update(updates);
                }
            }
            loadDashboard();
        }).catch((err) => {
    ...
    function checkAdminAccess() {
        if (!currentUserSessionData || !currentUserSessionData.subscription || !currentUserSessionData.subExpiry || currentUserSessionData.subExpiry <= Date.now()) {
            alert("Admin Panel kholne ke liye aapke paas Active Subscription hona zaroori hai!");
            return;
        }
    ...
    function buySubscription(cost, planName, extraDays, extraMins) {
        ...
        userRef.once('value', (snapshot) => {
            ...
            userRef.set(userData, (error) => {
                if(!error) {
                    alert(`Aapne '${planName}' successfully buy kar liya hai! Ab aap Admin Panel open kar sakte hain.`);
                } else {
    ...
    function renderAdminPointsTablesList(tablesObj) {
        ...
        let html = "<div style='font-size:12px; font-weight:bold; margin-bottom:5px;'>Bani hui Tables:</div>";
        keys.forEach(tName => {
